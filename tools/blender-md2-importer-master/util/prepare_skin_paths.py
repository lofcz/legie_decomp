...
def get_path_from_skin_name(md2_path: str, skin_name: str):
    ...
    # Clean up any garbage characters - split on common delimiters and take first part
    if "\x00" in first_stored_path:
        first_stored_path = first_stored_path.split("\x00")[0]
    ...
    first_stored_path = "".join(c for c in first_stored_path if c.isprintable())

    print(f"first_stored_path: {first_stored_path}")
    # absolute path is formed by using the given md2 object path
    absolute_first_stored_path = (
        "/".join(md2_path.split("/")[:-1]) + "/" + first_stored_path
    )
