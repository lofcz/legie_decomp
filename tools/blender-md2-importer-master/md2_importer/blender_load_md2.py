...
def blender_load_md2(
    md2_path, displayed_name, use_custom_md2_skin, custom_md2_skin_path
):
    """
    ...
    if use_custom_md2_skin:
        ...
        if os.path.isabs(custom_md2_skin_path):
            ...
            custom_abs_path = os.path.join(
                os.path.split(md2_path)[0], custom_md2_skin_path
            )
            print(custom_abs_path)
            skin_path = custom_abs_path
    else:
        print(
            "stored path:", my_object.skin_names
        )  # unchanged path or pathes stored in the MD2

        # skin_path = get_path_from_skin_name(object_path, my_object.skin_names[0])
        if my_object.skin_names and len(my_object.skin_names) > 0:
            skin_path = get_path_from_skin_name(object_path, my_object.skin_names[0])
        else:
            skin_path = None  # or handle no texture case
    ...
    all_verts = [
        [x.v for x in my_object.frames[y].verts]
        for y in range(my_object.header.num_frames)
    ]
    ...
    mesh.update()  # Update mesh so polygons are available for UV mapping
    ...
    # UV Mapping: Create UV Layer BEFORE checking skin path
    uv_layer = mesh.uv_layers.new()
    mesh.uv_layers.active = uv_layer

    # Determine which UV coordinates to use (will be needed even without texture)
    if skin_path and skin_path.endswith(".pcx"):
        uvs_to_use = uvs_pcx
    else:
        uvs_to_use = uvs_others

    print(f"Using PCX UVs: {skin_path and skin_path.endswith('.pcx')}")
    print(f"Skin path: {skin_path}")

    # Debug: Check texture coordinate ranges
    print(f"Number of texture coordinates: {len(my_object.texture_coordinates)}")
    print(f"Number of triangles: {len(my_object.triangles)}")

    # Check for any invalid texture indices
    for face_idx, tri in enumerate(my_object.triangles):
        for idx, tex_idx in enumerate(tri.textureIndices):
            if tex_idx < 0 or tex_idx >= len(my_object.texture_coordinates):
                print(
                    f"WARNING: Triangle {face_idx} has invalid texture index {tex_idx}"
                )

    # Assign UV coordinates to each polygon
    for face_idx, face in enumerate(mesh.polygons):
        for idx, (vert_idx, loop_idx) in enumerate(
            zip(face.vertices, face.loop_indices)
        ):
            tex_idx = my_object.triangles[face_idx].textureIndices[idx]
            if face_idx < 5:  # Debug first few faces
                print(
                    f"Face {face_idx}, vertex {idx}: tex_idx={tex_idx}, uv={uvs_to_use[tex_idx]}"
                )
            uv_layer.data[loop_idx].uv = uvs_to_use[tex_idx]

    # NOW check for skin path
    if not skin_path:
        ShowMessageBox(
            "UVs created but no skin found. You can manually assign a texture.",
            "No skin found",
            "INFO",
        )
        return {"FINISHED"}
    ...
    print(f"Number of triangles in MD2: {len(my_object.triangles)}")
    print(f"Number of texture coordinates: {len(my_object.texture_coordinates)}")
    ...
    for face_idx, face in enumerate(mesh.polygons):
        for idx, (vert_idx, loop_idx) in enumerate(
            ...
            if skin_path.endswith(".pcx"):
                print("PCX LOADED")
                uv_layer.data[loop_idx].uv = uvs_pcx[
                    my_object.triangles[face_idx].textureIndices[idx]
                ]
            else:
                uv_layer.data[loop_idx].uv = uvs_others[
                    my_object.triangles[face_idx].textureIndices[idx]
                ]
    ...
    if skin_path.endswith(".pcx"):
        ...
        texImage.image = bpy.data.images.new(
            "MyImage", width=skin.size[0], height=skin.size[1]
        )
