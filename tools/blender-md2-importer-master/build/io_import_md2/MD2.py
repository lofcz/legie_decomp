...
def load_gl_commands(gl_command_bytes):
    ...
    while True:  # ends when mode is 0
        ...
        for i in range(num_verts):
            s_and_t = struct.unpack(
                "<ff", gl_command_bytes[offset + 12 * i : offset + 12 * i + 8]
            )
            vertex_index = struct.unpack(
                "<i", gl_command_bytes[offset + 12 * i + 8 : offset + 12 * i + 12]
            )
...
def load_triangles(triangle_bytes, header):
    ...
    for i in range(header.num_tris):
        triangle = triangle_t(
            list(struct.unpack("<hhh", triangle_bytes[12 * i : 12 * i + 6])),
    ...
    for current_frame in range(header.num_frames):
        scale = vec3_t(
            *struct.unpack(
                "<fff",
                frames_bytes[
                    (40 + 4 * header.num_xyz) * current_frame : (
                        40 + 4 * header.num_xyz
                    )
                    * current_frame
                    + 12
        ...
        translate = vec3_t(
            *struct.unpack(
                "<fff",
                frames_bytes[
                    (40 + 4 * header.num_xyz) * current_frame + 12 : (
                        40 + 4 * header.num_xyz
                    )
                    * current_frame
                    + 24
        ...
        name = frames_bytes[
            (40 + 4 * header.num_xyz) * current_frame + 24 : (40 + 4 * header.num_xyz)
            * current_frame
            + 40
        ].decode("ascii", "ignore")
        verts = list()
            ...
            verts.append(
                vertex_t(
                    ...
                    *struct.unpack(
                        "<B",
                        frames_bytes[
                            (40 + 4 * header.num_xyz) * current_frame + 43 + v * 4 : (
                                40 + 4 * header.num_xyz
                            )
                            * current_frame
                            + 44
...
def load_texture_coordinates(texture_coordinate_bytes, header):
    ...
    for i in range(header.num_st):
        texture_coordinates.append(
            textureCoordinate_t(
                *struct.unpack("<hh", texture_coordinate_bytes[4 * i : 4 * i + 4])
            )
        )
...
def load_file(path):
    ...
    skin_names = [
        byte_list[header.ofs_skins + 64 * x : header.ofs_skins + 64 * x + 64].decode(
            "ascii", "ignore"
        )
    ...
    texture_coordinates = load_texture_coordinates(
        byte_list[header.ofs_st : header.ofs_tris], header
    )
    ...
    model = md2_object(
        header, skin_names, triangles, frames, texture_coordinates, gl_commands
    )
    return model
